...
MetaLibrariesConst.PlayerMenu = {}

local function pif(public, name, path)
  local private = {}
  private.Menu = util.CreateMetaTable('Menu')
  private.Button = util.CreateMetaTable('Button')

  function public.Create(ply, title)
    local Table = util.CopyMetaTable('Menu')
    ...
    MetaLibrariesConst.PlayerMenu[ply:UserID()][Table.Settings.Title] = Table

    return Table
  end

  function public.Find(ply, title)
    for k, v in pairs(MetaLibrariesConst.PlayerMenu[ply:UserID()]) do
  ...
  function public.Send(ply, title, page)
    ...
    hook.Add("menu", "MenuCore", function(ply, title, button)
      ...
      if button == 0 and Table.Control.Close ~= nil then
        ...
        page = Table.Control.Buttons[page][button]
        if page ~= nil then
          hook.Remove("menu", "MenuCore")
          page.func(ply, page.data)
        end
      end
  ...
  function public.Button(menu, page, button)
    local Table = CopyMetaTable('Button')
  ...
  function private.Button:Data(data)
    table.insert(self.Menu.Control.Buttons[self.Page][self.Button].data, data)
  end

  function private.Menu:SetControlClose(func)
  ...
  function private.Menu:AddButton(text, extra, func, ...)
    ...
    local button = public.Button(self, #self.Control.Buttons, #self.Control.Buttons[#self.Control.Buttons])
    --for _, v in pairs({...}) do
    --  button:Data(v)
    --end
  ...
  function private.Menu:SetPlayer(ply)
    self.Settings.Player = ply
  end

  function private.Menu:Send()
    public.Send(self.Settings.Player, self.Settings.Title)
  end

  mod_hook.Add('join', 'PlayerAuthJoin', function(ply)
    MetaLibrariesConst.PlayerMenu[ply:UserID()] = {}
  end)
end
